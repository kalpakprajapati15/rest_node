...
exports.watchMessage = (socket) => {
    ...
    socket.on('Client Message', (clientMessage) => {
        User.findOne({ _id: clientMessage.fromId }).then(userObj => {
            if (userObj) {
                this.handleMessage(clientMessage, userObj);
            }
        })
    })
}

this.handleMessage = (clientMessage, fromUser) => {
    User.findOne({ _id: clientMessage.toId }).then(toUser => {
        if (fromUser && toUser) {
            ...
            require('../socket').getIO().to(toUser.socketId).emit('Client Get Message', clientMessage)
...
exports.getMessages = (req, res, next) => {
    ...
    Message.find().or([{ $and: [{ fromId: fromId }, { toId: user._id }] },
                       ...
                       sort({ $natural: 1 }).limit().exec().then(messages => {
